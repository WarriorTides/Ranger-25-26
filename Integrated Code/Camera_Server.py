...
for i, cam_idx in enumerate(CAMERA_INDICES):
    ...
    if not cap.isOpened():
        ...
        print(
            f"    ✅ Camera {cam_idx} initialized: {actual_w}x{actual_h} @ {actual_fps}fps")
...
def camera_loop(cam_idx, cap):
    ...
    if cap is None or not cap.isOpened():
        print(
            f"❌ Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) thread exiting - camera not available")
        return
    ...
    print(
        f"🎥 Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) streaming started")
    ...
    while True:
        try:
            ...
            if not cap.isOpened():
                print(
                    f"❌ Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) lost connection")
                break
            ...
            if ret and frame is not None:
                ...
                if current_time - last_status_time >= 5.0:
                    fps = frame_count / \
                        (current_time - last_status_time + 0.001)
                    print(
                        f"📊 Camera {cam_idx + 1}: {frame_count} frames ({fps:.1f} fps)")
                    frame_count = 0
                ...
                if error_count % 30 == 0:  # Print every 30 errors
                    print(
                        f"⚠️  Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) failed to read frame (error #{error_count})")

            # Exit if too many consecutive errors
            if error_count > 100:
                print(
                    f"❌ Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) too many errors, stopping")
                break
            ...
            print(
                f"❌ Camera {CAMERA_INDICES[cam_idx]} (ID {cam_idx + 1}) error: {e}")
            error_count += 1
            time.sleep(0.1)
